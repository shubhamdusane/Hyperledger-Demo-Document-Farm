...
app.post('/api/pushDocumentUpload', async (req, res) => {
	...
	try {
	  const { documentID, contributorKey, dateTimeStamp } = req.body;
	  const result = await contract.submitTransaction('pushDocumentUpload', documentID, contributorKey, dateTimeStamp);
	  console.log('Transaction hash:', result.toString());
	  res.status(200).json({ transactionHash: result.toString() });
	} catch (error) {
